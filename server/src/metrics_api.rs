...
async fn get_metadata(db: web::Data<Db>, query: web::Query<ChannelQuery>) -> HttpResponse {
    ...
    let channels = vec![
        ChannelDef { id: "leaderboard".into(), name: "Levels Completed".into(), description: "Most distinct levels completed per player.".into(), score_unit: "Levels".into(), sort_order: "DESC".into() },
        ChannelDef { id: "interactions".into(), name: "Level Completions".into(), description: "Total level completions per player.".into(), score_unit: "Completions".into(), sort_order: "DESC".into() },
        ChannelDef { id: "boxes_broken".into(), name: "Boxes Broken".into(), description: "Total boxes broken across all levels.".into(), score_unit: "Boxes".into(), sort_order: "DESC".into() },
        ChannelDef { id: "power_ups_collected".into(), name: "Power-Ups Collected".into(), description: "Total power-ups collected across all levels.".into(), score_unit: "Power-Ups".into(), sort_order: "DESC".into() },
        ChannelDef { id: "speed_boosts".into(), name: "Speed Boosts".into(), description: "Total speed boosts used across all levels.".into(), score_unit: "Boosts".into(), sort_order: "DESC".into() },
    ];

    HttpResponse::Ok().json(MetricsResponse {
        name: "Heavy Ball".into(),
        description: "A physics-based puzzle game where players navigate colored balls through levels.".into(),
        achievements,
        channels,
    })
}
...
async fn get_user_profile(
    ...
    let (player_id, alias, wallet, _network_id) = resolved;
    ...
    let channels_data = if let Some(ref channel_ids) = query.channel {
        ...
        for ch in channel_ids {
            let (score, rank) = match ch.as_str() {
                "leaderboard" => {
                    let s = db.player_aggregate_f64(player_id, "levels_completed").unwrap_or(0.0);
                    let r = db.player_rank_in_channel(player_id, "leaderboard").unwrap_or(0);
                    (s, r)
                }
                "interactions" => {
                    let s = db.player_aggregate_f64(player_id, "level_completions").unwrap_or(0.0);
                    (s, 0)
                }
                "boxes_broken" => (db.player_aggregate_f64(player_id, "boxes_broken").unwrap_or(0.0), 0),
                "power_ups_collected" => (db.player_aggregate_f64(player_id, "power_ups_collected").unwrap_or(0.0), 0),
                "speed_boosts" => (db.player_aggregate_f64(player_id, "speed_boosts").unwrap_or(0.0), 0),
                _ => continue,
            };
