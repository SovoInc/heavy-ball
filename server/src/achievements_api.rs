...
async fn public_list(db: web::Data<Db>, query: web::Query<Prc1ListQuery>) -> HttpResponse {
    ...
    let achievements: Vec<Prc1Achievement> = ACHIEVEMENTS.iter().filter(|a| {
...
async fn wallet_achievements(db: web::Data<Db>, path: web::Path<String>) -> HttpResponse {
    ...
    let (player_id, alias, wallet_addr, network_id) = resolved;
    ...
    for def in ACHIEVEMENTS {
        ...
        let completed_rate = def.total.map(|total| {
            let progress = match def.name {
                "getting_started" | "halfway_there" | "completionist" => {
                    db.player_aggregate_f64(player_id, "levels_completed").unwrap_or(0.0)
                }
                "speed_demon" => db.player_aggregate_f64(player_id, "best_time").unwrap_or(0.0),
                "box_smasher" | "demolition_expert" => {
                    db.player_aggregate_f64(player_id, "boxes_broken").unwrap_or(0.0)
                }
                "power_collector" => db.player_aggregate_f64(player_id, "power_ups_collected").unwrap_or(0.0),
                "persistent" => db.player_aggregate_f64(player_id, "fall_count").unwrap_or(0.0),
                _ => 0.0,
            };
